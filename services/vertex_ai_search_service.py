...
class VertexAISearchService:
    ...
    def search_financial_info(self, address: str, page_size: int = 5) -> Dict[str, Any]:
        ...
        try:
            # 検索クエリを構築（財務状況に特化）
            query_text = f"以下の複数のデータソースをもとに、{address}の周辺環境を評価・要約してください。\n\n目的:\nユーザーがその場所に住むかどうかを判断するために、生活環境や安全性、子育て支援、公共インフラ、地域の印象を多角的に知りたいと考えています。\n\n出力形式:\n下記カテゴリに分けて、各カテゴリごとに簡潔かつ実用的なサマリを日本語で記載してください。\n\n【災害リスク】液状化、津波、洪水など（bousai）\n【医療・福祉】周辺の診療所・福祉施設（byouin, kankyou）\n【治安】刑法犯認知件数、陳情件数（chian）\n【環境・公害】大気汚染、公害苦情件数（kankyou）\n【子育て】保育園空き状況、学童、子育て広場、学校、公園（kosodate）\n【資産価値・地価】用途別土地価格（chika）\n【生活利便性】ゴミの分別情報、図書館、公衆浴場など（gyousei, kankyou）\n【地域の声】定住意向、人口推移（setai）\n\n制約:\n関連するファイルから対象地点に最も近い値または該当エリアの値を抽出すること\n形式はMarkdown形式のテーブルまたは見出し付きの自然文で\n出力は日本語で簡潔に\nデータが存在しない場合は「該当情報なし」と明記"
...
データが不足している場合は「データ不足」と記載してください。"""
    ...
    def _search_financial_info_fallback(self, address: str, page_size: int = 5) -> Dict[str, Any]:
        ...
        """
        try:
            query = f"以下の複数のデータソースをもとに、{address}の周辺環境を評価・要約してください。\n\n目的:\nユーザーがその場所に住むかどうかを判断するために、生活環境や安全性、子育て支援、公共インフラ、地域の印象を多角的に知りたいと考えています。\n\n出力形式:\n下記カテゴリに分けて、各カテゴリごとに簡潔かつ実用的なサマリを日本語で記載してください。\n\n【災害リスク】液状化、津波、洪水など（bousai）\n【医療・福祉】周辺の診療所・福祉施設（byouin, kankyou）\n【治安】刑法犯認知件数、陳情件数（chian）\n【環境・公害】大気汚染、公害苦情件数（kankyou）\n【子育て】保育園空き状況、学童、子育て広場、学校、公園（kosodate）\n【資産価値・地価】用途別土地価格（chika）\n【生活利便性】ゴミの分別情報、図書館、公衆浴場など（gyousei, kankyou）\n【地域の声】定住意向、人口推移（setai）\n\n制約:\n関連するファイルから対象地点に最も近い値または該当エリアの値を抽出すること\n形式はMarkdown形式のテーブルまたは見出し付きの自然文で\n出力は日本語で簡潔に\nデータが存在しない場合は「該当情報なし」と明記"
